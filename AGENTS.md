...
| `midi_clock_out.{h,c}` + `midi_clock_out_io.{h,cpp}` | LNK-027 Link→USB-MIDI clock OUT; `midi_clock_out.{h,c}` is a thin 24-PPQN adapter over `clock_ticker`, `midi_clock_out_io.cpp` is the 1ms FreeRTOS task + TinyUSB/DIN writes. ARC-024: carries the tick-health probe (gap/work/overruns/bursts/dropped/core + per-stage split), published as plain scalars and **never logged** — an `ESP_LOGx` in a 1 ms RT task is a blocking UART write (P4-033). Read out via `/status` |

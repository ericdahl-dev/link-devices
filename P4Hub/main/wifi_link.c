...
static int64_t s_connect_start_us = 0;

// SoftAP config fallback — reached either from a first-boot empty ssid, or
// (P4-018) from give_up_and_start_ap() once wifi_fallback_should_give_up()
// trips. Does not re-call esp_wifi_init()/event-handler registration —
// wifi_link_start() already did that once, regardless of which path is taken.
static void start_ap(void)
{
    ...
    ESP_LOGW(TAG, "SoftAP 'P4Hub-Setup' at 192.168.4.1 for config");
}

// P4-018: parity with X32Link's wifi_try_connect()/start_config_ap() — give
// STA a fixed budget (wifi_fallback.h) instead of retrying esp_wifi_connect()
// forever when the configured network can't be found/joined. Terminal until
// reboot, same as X32Link: once here, on_wifi_event() won't see STA events
// again (we've stopped STA and switched modes).

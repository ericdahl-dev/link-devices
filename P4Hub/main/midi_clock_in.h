...
// usb_midi_host.c calls midi_clock_in_pulse() from its IN callback; the P4-011
// master task reads midi_clock_in_bpm()/_pulse_count() to publish tempo to Link.
#include <stdint.h>
#include <stdbool.h>
#ifdef __cplusplus
extern "C" {
...
// bakes in "window intervals == one beat", so this MUST be 24 for correct BPM
// (a wider window would scale the tempo). us-precise esp_timer timestamps on the
// P4 make one beat of averaging enough; jitter smoothing isn't needed as on the S3.
